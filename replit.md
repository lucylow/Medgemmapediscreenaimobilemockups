...
## Mock Data System (2,500+ Scenarios)
- **Child Profiles** (`src/mock-data/child-profiles.ts`): 109 diverse child profiles with sex, language, region spanning East Africa, West Africa, Southern Africa, South Asia, East Asia, MENA, Latin America. 25 CHW names, 15 clinic locations including NICU follow-up, refugee posts, outreach tents. Seeded RNG utilities for deterministic generation.
- **Clinical Observations** (`src/mock-data/observations.ts`): Domain-specific observations across 9 age bands (0-3mo through 49-60mo) for 6 developmental domains (communication, gross_motor, fine_motor, problem_solving, personal_social, rop_screening). Each domain has 4 severity levels (on_track, monitor, concern, red_flag) with clinically accurate observation text. Includes ROP ETROP classification observations (preterm_extreme and preterm_moderate categories), parent report templates with placeholder interpolation ({WORDS}, {AGE}), M-CHAT-R flag descriptions (high_risk/moderate_risk/low_risk), ICD-10 code mappings (including H35.1x for ROP), and evidence-graded recommendations.
- **Screening Scenarios** (`src/mock-data/screening-scenarios.ts`): Generates 2,500 deterministic screening scenarios with seeded RNG. Domain-weighted distribution: communication 32%, motor 26% (gross 13% + fine 13%), personal-social 22%, problem solving 12%, ROP screening 8%. Each scenario includes: child demographics, domain-specific observations, CHW notes, risk levels (40% on_track, 30% monitor, 18% urgent, 12% referral), ASQ-3 scores/percentiles, ICD-10 codes, key findings, clinical summaries, recommendations with priority/timeline/evidence, domain breakdowns, preemie cases with corrected age, parent report narratives, M-CHAT-R scores/flags (ages 16-30mo for communication/social domains), ETROP classifications (zone/stage/plus disease/tortuosity/dilation for ROP), timestamps spanning 2 years, and model inference metadata.
- **Data Integration** (`src/app/data/mockData.ts`): Re-exports scenario data as Patient records, computes aggregate statistics (risk counts, domain counts including ROP, age distribution, avg confidence/inference time), feeds Dashboard mock dataset card and Impact Dashboard.
- **Utility Functions**: `getScenariosByRisk()`, `getScenariosByDomain()`, `getScenariosByAgeRange()`, `getScenarioStats()`, `getRecentScreenings()` for filtering and analytics. Stats now include ropCount, mchatRCount, parentReportCount, correctedAgeCount.
